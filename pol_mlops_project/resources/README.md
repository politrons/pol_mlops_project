...
The databricks CLI bundles top level is defined by file `pol_mlops_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`pol_mlops_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`pol_mlops_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`pol_mlops_project/resources/monitoring-resource.yml`)
 - feature engineering workflow (`pol_mlops_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`pol_mlops_project/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `pol_mlops_project/databricks.yml` and `pol_mlops_project/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-pol_mlops_project-model     | /dev-pol_mlops_project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-pol_mlops_project-model | /staging-pol_mlops_project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-pol_mlops_project-model    | /prod-pol_mlops_project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-pol_mlops_project-model    | /test-pol_mlops_project-experiment    |
...
2. Complete the `TODO` in `pol_mlops_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `pol_mlops_project/resources/batch-inference-workflow-resource.yml` to pass
...
* [model-workflow-resource.yml](training/training-model-fe-workflow-resource.yml) contains the resource config and input parameters for model validation.
...
resolve the `TODOs` in the ModelValidation task of [model-workflow-resource.yml](training/training-model-fe-workflow-resource.yml).
...
* [monitoring-resource.yml](monitoring/monitoring-resource.yml) contains the resource config, inputs parameters for monitoring, and orchestrates model retraining based on monitoring. It first runs the [notebooks/MonitoredMetricViolationCheck](../monitoring/notebooks/MonitoredMetricViolationCheck.py)
entry point then decides whether to execute the model retraining workflow.

To set up and enable monitoring:
* If it is not done already, generate inference table, join it with ground truth labels, and update the table name in [monitoring-resource.yml](monitoring/monitoring-resource.yml).
* Resolve the `TODOs`  in [monitoring-resource.yml](monitoring/monitoring-resource.yml)
...
To get started, open `pol_mlops_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
The example above defines a Databricks job with name `${bundle.target}-pol_mlops_project-batch-inference-job`
that runs the notebook under `pol_mlops_project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-pol_mlops_project-batch-inference-job`.
...
`test-pol_mlops_project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-pol_mlops_project-batch-inference-job`.
